...
public class EmpController {
	...
	public String selectEmpList(EmpVO vo, Model model ) throws Exception {
		...
		model.addAttribute("totalPage",totalPage);  //총페이지 번호
		model.addAttribute("total",total);		//총 데이터 값
		model.addAttribute("list",list);		// 페이지 출력 목록
		model.addAttribute("rownum",rownum);	//출력 페이지 행 시작번호
	...
	public String updateEmpModify(EmpVO vo) throws Exception{	
	...
	// 월/일/년 => 년/월/일
		...
		int result = empService.UpdateEmpDetail(vo);
		return "redirect:empList.do";
	}
